              ...
              <div className="space-y-2 mb-6">
                ...
                {calculateSavings() > 0 && (
                  <div className="text-green-600 font-semibold">
                    You save {currencySymbols[product.currency] || '€'}{calculateSavings()} ({product.discount}%)
                  </div>
                )}
                    ...
                    <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-600">
                          {option.toLowerCase() === 'shipping' ? 'Free delivery in 2-3 business days' : 'Available for pickup'}
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="space-y-3">
                ...
                {product.available_from && (
                  <div className="flex justify-between py-2 border-b border-gray-100">
                    ...
                    <span className="font-medium">{product.available_from}</span>
                  </div>
                )}
                <div className="flex justify-between py-2 border-b border-gray-100">
                  <span className="text-gray-600">Currency</span>
                  <span className="font-medium">{product.currency || 'EUR'}</span>
                </div>
                {product.timestamp && (
                  <div className="flex justify-between py-2">
                    <span className="text-gray-600">Listed On</span>
                    <span className="font-medium">{new Date(product.timestamp).toLocaleDateString()}</span>
                  </div>
                )}
              </div>
            </div>
